...
export class TikTokHandler {
    ...
    async parse(url: string, cookies?: string): Promise<ParsedVideo> {
        return new Promise((resolve, reject) => {
            logger.info(`Parsing TikTok URL with yt-dlp: ${url}`);
            ...
            childProcess.on('close', (code) => {
                if (code !== 0) {
                    logger.error(`TikTok yt-dlp error: ${stderr}`);
                    ...
                    reject(new Error(stderr || 'Failed to parse TikTok content'));
                    return;
                }
            ...
            childProcess.on('error', (err) => {
                logger.error(`TikTok yt-dlp spawn error: ${err.message}`);
    ...
    private transformInfo(info: any): ParsedVideo {
        ...
        // Video formats - prefer no-watermark
